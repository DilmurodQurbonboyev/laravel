...
<head>
    ...
    <link rel="icon" href="{{ asset('img/gerb.png') }}">
...
<script>
    ...
    $('.deleteBtn').click(function (event) {
        ...
        event.preventDefault();
        Swal.fire({
            title: "{{ M::t('Are you sure') }} ?",
            text: "{{ M::t('This information will be deleted') }}!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
            cancelButtonColor: '#6c757d',
            confirmButtonText: "{{ M::t('Delete') }}",
            cancelButtonText: "{{ M::t('Cancel') }}",
        }).then((result) => {
            if (result.isConfirmed) {
                form.submit();
                Swal.fire({
                    title: "{{ M::t('Successfully deleted') }}",
                    icon: 'success',
    ...
    $('.forceDelete').click(function (event) {
        ...
        Swal.fire({
            title: "{{ M::t('Are you sure') }} ?",
            text: "{{ M::t('You can not restore this information') }}!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
            cancelButtonColor: '#6c757d',
    ...
    $('.restore').click(function (event) {
        ...
        Swal.fire({
            title: "{{ M::t('Are you sure') }} ?",
            text: "{{ M::t('This information will be restored') }}!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
            cancelButtonColor: '#6c757d',
            confirmButtonText: "{{ M::t('Restore') }}",
            cancelButtonText: "{{ M::t('Cancel') }}",
        }).then((result) => {
            if (result.isConfirmed) {
                form.submit();
                Swal.fire({
                    title: "{{ M::t('Successfully restored') }}",
                    icon: 'success',
                    showConfirmButton: false,
                    timer: 2500
                })
